...
### 아이디어
 ...
 <p> 원생들을 오름차순으로 정렬한뒤 사이사이 차이값을 계산한다. 기본적으로 K조에 1명씩 들어가야되므로(1명이 들어가는것이 최소값이기도함) N-K의 인원이 남는다. 나머지 인원은 차이값을 오름차순으로 정렬한 리스트에서 남은 인원만큼 값을 빼서 더하고 출력한다. </p>
 
 <p> 차이값을 계산하고 나서 바로 옆에 붙어있는 숫자의 계산에 대해서는 둘이 합친 값이 결국 가장 큰 숫자에서 작은 숫자를 뺀 값이 되므로 작은 숫자 순으로 빼도 상관이 없다. </p>
